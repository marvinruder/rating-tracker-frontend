node {
    withEnv([
        ...
        stage ('Run Tests') {
            ...
            sh """
            docker run -a STDOUT -a STDERR $imagename:build-$GIT_COMMIT_HASH-test
            docker run -v \$(pwd)/coverage:/coverage -a STDOUT -a STDERR $imagename:build-$GIT_COMMIT_HASH-test cp /app/coverage/* /coverage
